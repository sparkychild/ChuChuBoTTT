     ...
     ambush: function(arg, by, room) {
         ...
         switch (action) {
             ...
             case 'start':
                 ...
                 Bot.say(by, room, 'The game is starting! Use ' + config.commandcharacter[0] + 'fire [name] to target another player after I say "FIRE".')
                 Bot.say(by, room, 'Round 1. | Players (' + Object.keys(ambush.data[room]).length + '): ' + Object.keys(ambush.data[room]).join(', '));
                 postFire(room);
                 break;
             case 'end':
                 if (!Bot.canUse('signups', room, by) || !ambush.state[room] || ambush.state[room] === 'off') return false;
                 clearTimer(room);
                 ambush.state[room] = 'off';
             ...
             case 'players':
                 if (!Bot.canUse('signups', room, by) || !ambush.state[room] || ambush.state[room] === 'off') return false;
                 Bot.say(config.nick, room, 'Players (' + Object.keys(ambush.data[room]).length + '): ' + Object.keys(ambush.data[room]).join(', '));
                 break;
         }
     },

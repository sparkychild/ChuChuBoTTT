...
function getTiers(array) {
    var tiers = []
    for (var i = 0; i < array.length; i++) {
        try {
            var tarTier = pokemonData[array[i]].tier.replace(/[\(\)]/g, '');
        } catch (e) {
            return tiers.join('/');
        }
        if (tiers.indexOf(tarTier) > -1) continue;
        tiers.push(tarTier);
    }
    return tiers.join('/');
}
exports.commands = {
    ...
    statspread: function(arg, by, room, cmd) {
        ...
        var statSpread = POKEDEX[tarMon].baseStats;
        statspread.answer[room] = findSimilarBST(statSpread);
        Bot.say(by, room, 'Stat Spread: ' + Object.values(statSpread).join('/') + ' [' + getTiers(statspread.answer[room]) + ']. Use ' + config.commandcharacter[0] + 'g to submit your answer.');
    },
    guessstatspread: function(arg, by, room) {
        if (!arg || !statspread.on[room]) return false;
        arg = toId(arg);
